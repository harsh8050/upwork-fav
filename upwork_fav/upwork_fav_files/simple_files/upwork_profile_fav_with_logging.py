...
def fetch_data():
   try : 
       job_activity = driver.execute_script("return window.__NUXT__.state.jobDetails.job.clientActivity")
       if job_activity:
...
def fetch_hourly_ids():
    cursor = conn.cursor()
    query = f"""
        SELECT profile_url_id FROM {upwork_profiles}
        WHERE 
            ...
            AND (
                (
                    DATE(posted_on) = CURDATE()
                    AND (
                        last_scraped_at IS NULL
                        OR TIMESTAMPDIFF(MINUTE, last_scraped_at, NOW()) >= 60
                    )
                )
                OR (
                    DATE(posted_on) = CURDATE() - INTERVAL 1 DAY
                    AND (
                        last_scraped_at IS NULL
                        OR TIMESTAMPDIFF(MINUTE, last_scraped_at, NOW()) >= 120
                    )
                )
                OR (
                    DATE(posted_on) >= CURDATE() - INTERVAL 7 DAY
                    AND DATE(posted_on) < CURDATE() - INTERVAL 1 DAY
                    AND (
                        last_scraped_at IS NULL
                        OR TIMESTAMPDIFF(MINUTE, last_scraped_at, NOW()) >= 360
                    )
                )
...
def fetch_daily_ids():
    cursor = conn.cursor()
    query = f"""SELECT profile_url_id FROM {upwork_profiles}
        WHERE (is_favourite = 1 OR is_bid = 1) AND is_closed = 0 AND is_removed IN (3,4) AND is_rejected = 0
        AND DATE(posted_on) < CURDATE() - INTERVAL 7 DAY
        AND (
            last_scraped_at IS NULL
    ...
    try:
        ...
        if id_type == "hourly" : 
            ...
            SET 
                ...
                d.job_activity = %s,
                p.last_scraped_at = %s,
                p.send_notification = CASE
                    WHEN (
                        DATE(p.posted_on) = CURDATE()
                        AND (
                            p.last_scraped_at IS NULL
                            OR TIMESTAMPDIFF(MINUTE, p.last_scraped_at, NOW()) >= 60
                        )
                        AND d.last_viewed_by_client BETWEEN NOW() - INTERVAL 1 HOUR AND NOW()
                    )
                    THEN 3

                    WHEN (
                        DATE(p.posted_on) = CURDATE() - INTERVAL 1 DAY
                        AND (
                            p.last_scraped_at IS NULL
                            OR TIMESTAMPDIFF(MINUTE, p.last_scraped_at, NOW()) >= 120
                        )
                        AND d.last_viewed_by_client BETWEEN NOW() - INTERVAL 2 HOUR AND NOW()
                    )
                    THEN 3

                    WHEN (
                        DATE(p.posted_on) >= CURDATE() - INTERVAL 7 DAY
                        AND DATE(p.posted_on) < CURDATE() - INTERVAL 1 DAY
                        AND (
                            p.last_scraped_at IS NULL
                            OR TIMESTAMPDIFF(MINUTE, p.last_scraped_at, NOW()) >= 360
                        )
                        AND d.last_viewed_by_client BETWEEN NOW() - INTERVAL 6 HOUR AND NOW()
                    )
                    THEN 3

                    ELSE p.send_notification
                END
            ...
            SET 
                ...
                p.send_notification = CASE
                    WHEN 
                    d.last_viewed_by_client >= (NOW() - INTERVAL 24 HOUR)
                    THEN 3
                    ELSE p.send_notification
                END
            WHERE p.profile_url_id = %s 
        ...
        conn.commit()
        cursor.execute(f'SELECT send_notification FROM {upwork_profiles} WHERE profile_url_id = %s', (id,))
        status = cursor.fetchone()  # You had `fetch()`, it should be `fetchone()`
        
        if status and status[0] == 3:
            print("✅ Notification sent.")
            print("old status : ",old_status[0],old_view[0])
            print("New status : ",status[1])
            print("📄 Executed Query:")
            print(format_sql(update_query, params))
    except:
        traceback.print_exc()
    finally:
        if cursor:
...
def execute(id,id_type):
    ...
    try : 
        try : 
           status = driver.execute_script("return window.__NUXT__.state.jobDetails.job.status")
           if status != 1 : 
                set_is_closed(id)
                set_is_removed(id,1)
                print("Job Closed")
            ...
            try : 
                status = driver.execute_script("return window.__NUXT__.state.jobDetails.job")
                if not status : 
                    set_is_closed(id)
                    set_is_removed(id,1)
                    print("Job Acess Denied")
                    return
            except :  
                set_is_closed(id)
                set_is_removed(id,1)
                ...
                return
        try :
            status = driver.find_element(By.CSS_SELECTOR, '#main > div.container > div:nth-child(4) > div > div:nth-child(2) > div > div.span-md-9.reason-text > h4').text.strip()
            print("status :", status)
        except : 
            status = driver.find_element(By.XPATH,'//*[@id="main"]/div[3]/div[4]/h1').text.strip()
            print("status :", status)
        # 1 : no longer available or not found,  2 : denied, 3 : private,
